...
import { password_hash_uses_bcrypt } from "@/lib/server/password";
import {
  ApiError,
  api_error,
  api_ok,
...
export async function POST(request: NextRequest, route_context: RouteContext) {
  try {
    ...
    if (action === "refresh") return await refresh(request);
    if (action === "logout") return await logout(request);
    if (action === "tenant-context") return await tenant_context(request);
    if (action === "forgot-password") return await forgot_password(request);
    if (action === "reset-password") return await reset_password(request);
    throw new ApiError(404, "not_found", "Endpoint tidak ditemukan");
  } catch (error) {
    return api_error(error);
  }
...
async function login(request: NextRequest) {
  ...
  if (
    !user ||
    user.status !== "active" ||
    !(await verify_password(user.passwordHash, input.password))
  ) {
    throw new ApiError(401, "invalid_login", "Email atau password salah");
  }
  if (!password_hash_uses_bcrypt(user.passwordHash)) {
    await prisma.user.update({
      where: { id: user.id },
      data: { passwordHash: await hash_password(input.password) },
    });
  }
  const initial_membership = user.isSuperAdmin
    ? undefined
    : user.memberships[0];
  if (!user.isSuperAdmin && !initial_membership)
    throw new ApiError(
      403,
      "tenant_inactive",
      "Akun tidak memiliki tenant aktif",
    );
  const context: AuthContext = {
    ...
    role: user.isSuperAdmin
      ? "super_admin"
      : (initial_membership?.role as AuthContext["role"]),
  };
...
async function refresh(request: NextRequest) {
  ...
  const refresh_token =
    input.refresh_token ?? request.cookies.get("refresh_token")?.value;
  if (!refresh_token)
    throw new ApiError(
      401,
